...
Room* FakeDataLoader::fakeRoom = nullptr;
...
Room* FakeDataLoader::buildFakeRoom(PlayableCharacter* character)
{
    string room1Key = "jerrys_garage";
    string room1Intro = "On a summer day in the hot August heat, you've finally reached the back of the moving truck. There's"
                        "not much room left in the house, so you decide to bring the last remaining boxes to the garage. Upon"
    ...
    fakeRoom = new Room(room1Key, room1Intro, room1Characters, room1Menus);
    // ACTIONS
    ...
    Action* crateSearch = new Search(character, crateSearchKey, fakeRoom, crateSearchContext, crateSearchConditions, scmekles);
    ...
    Action* shelfSearch = new Search(character, shelfSearchKey, fakeRoom, shelfSearchContext, shelfSearchConditions, crateSearchItems);
    ...
    Action* changeRoomCarpet = new ChangeRoom(character, changeRoomCarpetKey, fakeRoom, changeRoomCarpetContext, changeRoomCarpetConditions, changeRoomCarpetItems);
    ...
    Action* room1PrintInventory = new PrintInventory(character, room1PrintInventoryKey, room1PrintInventoryConditions, room1PrintInventoryItems);

    room1Actions[room1PrintInventory->getKey()] = room1PrintInventory;

    string room1SaveAndExitKey = "save_and_exit";

    Action* room1SaveAndExit = new SaveAndExit(character, room1SaveAndExitKey, fakeRoom);

    room1Actions[room1SaveAndExit->getKey()] = room1SaveAndExit;

    fakeRoom->setActions(room1Actions);
